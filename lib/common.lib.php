...
// paging URL reassembly(pretty url)
function paging_url_reassembly($url)
{
...
class str_encrypt
{
    ...
    function __construct($salt='')
    {
        ...
        $this->secret_key = md5(preg_replace('/[^0-9A-Za-z]/', '', $_SERVER['DOCUMENT_ROOT']));
        $this->secret_iv = substr($this->salt, 1, strlen($this->secret_key)).substr($this->secret_key, -1);
    }
    ...
    function encrypt($str)
    {
        ...
        $result = $this->add_encrypt_decrypt('encrypt', $result);
    ...
    function decrypt($str) {
        ...
        $str = $this->add_encrypt_decrypt('decrypt', $str);
        $length = strlen($str);
    ...
    function add_encrypt_decrypt($action, $string) {
        ...
        if( extension_loaded('openssl') && function_exists('openssl_encrypt') && function_exists('hash') ){
            $encrypt_method = "AES-256-CBC";

            $op_key = $op_key ? $op_key : hash('sha256', $this->secret_key);
            $op_iv = $op_iv ? $op_iv : substr(hash('sha256', $this->secret_iv), 0, 16);

            if ( $action === 'encrypt' ) {
                $output = openssl_encrypt($string, $encrypt_method, $op_key, 0, $op_iv);
            } else if( $action === 'decrypt' ) {
                $output = openssl_decrypt($string, $encrypt_method, $op_key, 0, $op_iv);
            }
        } else if ( defined('MCRYPT_DEV_URANDOM') && function_exists('mcrypt_create_iv') && function_exists('hash') ){
            $mc_key = $mc_key ? $mc_key : hash('sha256', $this->secret_key, true);

            if ( $action == 'encrypt' ) {
                $mc_iv = $mc_iv ? $mc_iv : mcrypt_create_iv(mcrypt_get_iv_size(MCRYPT_RIJNDAEL_128, MCRYPT_MODE_CBC), MCRYPT_DEV_URANDOM);
                $output = $mc_iv.mcrypt_encrypt(MCRYPT_RIJNDAEL_128, $mc_key, $string, MCRYPT_MODE_CBC, $mc_iv);
            } else if( $action == 'decrypt' ) {
                $decrypt_iv = substr($string, 0, mcrypt_get_iv_size(MCRYPT_RIJNDAEL_128, MCRYPT_MODE_CBC));
                $output = rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_128, $mc_key, substr($string, mcrypt_get_iv_size(MCRYPT_RIJNDAEL_128, MCRYPT_MODE_CBC)), MCRYPT_MODE_CBC, $decrypt_iv), "\0");
            }
        }
...
function adjust_url_to_config($url)
{
	...
	if(stripos($url, '.php') !== false && $config['cf_bbs_rewrite']) {
		$url_element = explode('=', $url);
		$id = end($url_element);

		if(stripos($url, 'bo_table') !== false) {
			$url = GML_URL. '/'. $id;
		} else if(stripos($url, 'gr_id') !== false) {
			$url = GML_URL. '/group/'. $id;
		} else if(stripos($url, 'co_id') !== false) {
			$url = GML_URL. '/content/'. $id;
		}
    // convert shortten url to original url
    } else if(stripos($url, '.php') === false && !$config['cf_bbs_rewrite']) {
		$url_element = str_replace(GML_URL, '', $url);
		$url_element2 = explode('/', $url_element);
		$id = str_replace('/', '', end($url_element2));

		if(stripos($url_element, 'content') !== false) {
			$url = GML_BBS_URL. '/content.php?co_id='. $id;
		} else if(stripos($url_element, 'group') !== false) {
			$url = GML_BBS_URL. '/group.php?gr_id='. $id;
		} else if((stripos($url, GML_URL) !== false) && $id) {
